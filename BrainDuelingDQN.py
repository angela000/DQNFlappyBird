...
LOGS_PATH = "./logs_" + GAME + "/dueling_dqn/"
SAVE_PATH = "./saved_parameters/dueling_dqn/"
SAVE_BACK_PATH = "./saved_back/dueling_dqn/"
...
def createNetwork():
    ...
    with tf.variable_scope("eval_net"):
        ...
        W_fc2_ea = tf.Variable(tf.truncated_normal([512, ACTIONS], stddev=0.01))
        b_fc2_ea = tf.Variable(tf.constant(0.01, shape=[1, ACTIONS]))
        eval_A = tf.matmul(h_fc1_e, W_fc2_ea) + b_fc2_ea

        readout_e = eval_V + (eval_A - tf.reduce_mean(eval_A, axis=1, keep_dims=True))  # Q = V(s) + A(s,a)
    ...
    with tf.variable_scope("target_net"):
        ...
        W_fc2_ta = tf.Variable(tf.truncated_normal([512, ACTIONS], stddev=0.01), trainable=False)
        b_fc2_ta = tf.Variable(tf.constant(0.01, shape=[1, ACTIONS]), trainable=False)
        target_A = tf.matmul(h_fc1_t, W_fc2_ta) + b_fc2_ta

        readout_t = target_V + (target_A - tf.reduce_mean(target_A, axis=1, keep_dims=True))  # Q = V(s) + A(s,a)
...
def trainNetwork(eval_net_input, target_net_input, readout_eval, readout_target, h_fc1_eval, h_fc1_target, sess):
    ...
    while "ZR love EV" != "False":  # A no-stop circulation.
        ...
        if t > OBSERVE:
            ...
            '''Double DQN'''
            # readout_j1_batch_action = readout_eval.eval(feed_dict={eval_net_input: s_j1_batch})  # (32, 2)
